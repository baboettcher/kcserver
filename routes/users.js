...
router.get("/all_districts", function(req, res, next) {
  ...
  District.find((err, districts) => {
    ...
    // send the list of all districts
...
router.post("/teacher", function(req, res, next) {
  ...
  console.log("NEW TEACHER REQ.BODY:  ", req.body);
  console.log("REMINDER: Errors not being send back. Change 'next?'");
  // method #2 - "Create" - creates new instance and saves all in one
...
// ------------ MONSTER EXAMPLE ---------- //
router.get("/all_monsters", function(req, res, next) {
  console.log("GET ALL MONSTERS");
  Monster.find((err, monsters) => {
    if (err) return res.status(500).send(err);
    // send the list of all monsters
    return res.status(200).send(monsters);
  });
});

router.post("/monster", function(req, res, next) {
  console.log("NEW MONSTER POST: ", req.body);
